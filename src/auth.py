...
def create_magic_token(email: str, expire_minutes: int = 15) -> str | None:
    """Create a magic token for a registered user. Returns token or None if user not found."""
...
def get_session_user(session_token: str) -> dict | None:
    """Return user dict (id, email, role) for a valid session token, or None."""
    now = _now_iso()
